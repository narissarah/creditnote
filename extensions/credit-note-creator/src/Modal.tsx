...
/**
 * Credit Note Creator Modal
 *
 * Flow:
 * 1. Create Screen - Collect customer info and credit details
 * 2. Success Screen - Display created credit note with QR code
 * 3. Print Dialog - Optional QR code printing
 *
 * Features:
 * - Form validation
...
function CreditNoteCreatorModal() {
  ...
  /**
   * Creates credit note by calling backend API
   */
  const handleCreate = async () => {
    ...
    try {
      ...
      // Convert dollars to cents for database
      ...
      // Call API to create credit note
  ...
  /**
   * Fetches QR code image for the created credit note
   */
  const fetchQRCode = async (creditNoteId: string) => {
    try {
      ...
      console.log('[Credit Creator] Fetching QR code for note:', creditNoteId);
      ...
      if (response.ok) {
        ...
        if (data.success && data.qrCodeImage) {
          ...
          console.log('[Credit Creator] ✅ QR code loaded successfully');
        }
      } else {
        console.error('[Credit Creator] Failed to fetch QR code:', response.status);
      }
    } catch (error) {
  ...
  /**
   * Confirms print action (placeholder for actual print API)
   */
  const confirmPrint = () => {
  ...
  /**
   * Resets form to create another credit note
   */
  const resetForm = () => {
    setCustomerName('');
    setCustomerEmail('');
