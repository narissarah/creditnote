...
// CRITICAL: Auto-migration for serverless environments
// This ensures tables exist even if migrations weren't run during build
...
export async function ensureTablesExist(): Promise<void> {
  ...
  try {
    console.log('[DB MIGRATION] Checking if tables exist...');

    // Check if Session table exists
    await prisma.$queryRaw`SELECT 1 FROM "Session" LIMIT 1`;
    console.log('[DB MIGRATION] ✅ All tables exist');
    migrationChecked = true;
  } catch (error) {
    console.log('[DB MIGRATION] Tables missing, creating them now...');
    ...
    try {
      ...
      // Create CreditNote table if it doesn't exist
      await prisma.$executeRawUnsafe(`
        ...
        CREATE INDEX IF NOT EXISTS "credit_notes_customerId_idx" ON "credit_notes"("customerId");
        CREATE INDEX IF NOT EXISTS "credit_notes_shopDomain_status_idx" ON "credit_notes"("shopDomain", "status");
        CREATE INDEX IF NOT EXISTS "credit_notes_metafieldId_idx" ON "credit_notes"("metafieldId");
        CREATE INDEX IF NOT EXISTS "credit_notes_orderId_idx" ON "credit_notes"("orderId");
      `);

      // Create CreditRedemption table
      await prisma.$executeRawUnsafe(`
        ...
        CREATE INDEX IF NOT EXISTS "credit_redemptions_creditNoteId_idx" ON "credit_redemptions"("creditNoteId");
        CREATE INDEX IF NOT EXISTS "credit_redemptions_orderId_idx" ON "credit_redemptions"("orderId");
      `);
      ...
      console.log('[DB MIGRATION] ✅ All tables created successfully');
      migrationChecked = true;
    } catch (createError) {
      console.error('[DB MIGRATION] Failed to create tables:', createError);
      // Don't throw - let the app continue and try again next time
    }
  }
}
